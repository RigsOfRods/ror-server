...
MyDialog::MyDialog(const wxString &title, MyApp *_app) :
    ...
    tb->AddTool(btn_start, _T("Start"), wxBitmap(play_xpm), wxBitmap(play_dis_xpm), wxITEM_NORMAL, _T("Start server"),
                _T("Start server"));
    tb->AddTool(btn_stop, _T("Stop"), wxBitmap(stop_xpm), wxBitmap(stop_dis_xpm), wxITEM_NORMAL, _T("Stop server"),
                _T("Stop server"));
    tb->AddSeparator();
    tb->AddTool(btn_exit, _T("Shutdown"), wxBitmap(shutdown_xpm), wxBitmap(shutdown_dis_xpm), wxITEM_NORMAL,
                _T("Shutdown server"), _T("Shutdown server"));
    tb->Realize();
    tb->EnableTool(btn_stop, false);


    // head image - using xpm
    wxStaticPicture *imagePanel = new wxStaticPicture(this, -1, wxBitmap(config_xpm), wxPoint(0, 0), wxSize(500, 100),
                                                      wxNO_BORDER);
    ...
    sname = new wxTextCtrl(settingsPanel, wxID_ANY, conv(RORNET_VERSION), wxDefaultPosition, wxDefaultSize,
                           wxALIGN_RIGHT);
    sname->Disable();
    grid_sizer->Add(sname, 0, wxGROW);

    // server name
    dText = new wxStaticText(settingsPanel, wxID_ANY, _("Server Name:"), wxDefaultPosition, wxDefaultSize,
                             wxALIGN_RIGHT);
    grid_sizer->Add(dText, 0, wxGROW);
    ...
    // logfilename
    dText = new wxStaticText(settingsPanel, wxID_ANY, _("Log Filename:"), wxDefaultPosition, wxDefaultSize,
                             wxALIGN_RIGHT);
    grid_sizer->Add(dText, 0, wxGROW);
    ...
    // adminuid
    dText = new wxStaticText(settingsPanel, wxID_ANY, _("Admin user token:"), wxDefaultPosition, wxDefaultSize,
                             wxALIGN_RIGHT);
    grid_sizer->Add(dText, 0, wxGROW);
    ...
    txtConsole = new wxTextCtrl(logPanel, wxID_ANY, _(""), wxDefaultPosition, wxDefaultSize,
                                wxNO_BORDER | wxTE_MULTILINE | wxTE_READONLY | wxTE_RICH | wxTE_RICH2 | wxTE_DONTWRAP);

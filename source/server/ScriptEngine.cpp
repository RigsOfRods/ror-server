...
std::string ServerScript::getUserAuth(int uid)
{
    ...
    if (c->user.authstatus & RoRnet::AUTH_ADMIN)
        return "admin";
    else if (c->user.authstatus & RoRnet::AUTH_MOD)
        return "moderator";
    else if (c->user.authstatus & RoRnet::AUTH_RANKED)
        return "ranked";
    else if (c->user.authstatus & RoRnet::AUTH_BOT)
        return "bot";
...
unsigned int ServerScript::get_maxClients()
{
    return Config::getMaxClients();
}

std::string ServerScript::get_serverName()
{
    return Config::getServerName();
}

std::string ServerScript::get_IPAddr()
{
    return Config::getIPAddr();
}

unsigned int ServerScript::get_listenPort()
{
    return Config::getListenPort();
}

int ServerScript::get_serverMode()
{
...
std::string ServerScript::get_website()
{
    return Config::getWebsite();
}

std::string ServerScript::get_ircServ()
{
    return Config::getIRC();
}

std::string ServerScript::get_voipServ()
{
    return Config::getVoIP();
}

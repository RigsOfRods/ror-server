...
bool Blacklist::LoadBlacklistFromFile()
{
    ...
    try
    {
        ...
        for (int i = 0; i < bans->size(); ++i)
        {
            ...
            Logger::Log(LogLevel::LOG_WARN, "AAAAAAAAAAA %s", j_ban->getValue<std::string>("nickname").c_str());
            m_database->RecordBan(j_ban->getValue<int>("bid"), j_ban->getValue<std::string>("ip"),

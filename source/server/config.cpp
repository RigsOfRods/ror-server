...
#define HANDLE_ARG_FLAG(_NAME_, _BLOCK_) \
    ...
    bool isPublic()
    {
        return !getPublicPassword().empty();
    }
    ...
    {
        return s_terrain_name;
    }

    const std::string &getPublicPassword()
    {
        return s_public_password;
    }
    ...
    bool getPrintStats()
    {
        return s_print_stats;
    }

    bool getWebserverEnabled()
    {
        return s_webserver_enabled;
    }

    unsigned int getWebserverPort()
    {
        return s_webserver_port;
    }
    ...
    {
        return s_serverlist_host;
    }

    const char *GetServerlistHostC()
    {
        return s_serverlist_host.c_str();
    }

    const std::string &GetServerlistPath()
    {
        return s_serverlist_path;
    }
    ...
    {
        return s_heartbeat_retry_count;
    }

    unsigned int GetHeartbeatRetrySeconds()
    {
        return s_heartbeat_retry_seconds;
    }

    unsigned int GetHeartbeatIntervalSec()
    {
        return s_heartbeat_interval_sec;
    }
    ...
    {
        s_webserver_port = port;
    }

    void setWebserverEnabled(bool webserver)
    {
        s_webserver_enabled = webserver;
    }
    ...
    void setRulesFile(const std::string &file)
    {
        s_rulesfile = file;
    }
...
#define VAL_BOOL(_STR_)    (RudeStrToBool(value))
    ...
    {
        if (strcmp(key, "baseconfig") == 0)
            LoadConfigFile(VAL_STR(value));
        else if (strcmp(key, "slots") == 0)
            setMaxClients(VAL_INT(value));
        else if (strcmp(key, "name") == 0)
            setServerName(VAL_STR(value));
        else if (strcmp(key, "scriptname") == 0)
            setScriptName(VAL_STR(value));
        else if (strcmp(key, "terrain") == 0)
            setTerrain(VAL_STR(value));
        else if (strcmp(key, "password") == 0)
            setPublicPass(VAL_STR(value));
        else if (strcmp(key, "ip") == 0)
            setIPAddr(VAL_STR(value));
        else if (strcmp(key, "port") == 0)
            setListenPort(VAL_INT(value));
        else if (strcmp(key, "mode") == 0)
            SetConfServerMode(VAL_STR(value));
        else if (strcmp(key, "printstats") == 0)
            setPrintStats(VAL_BOOL(value));
        else if (strcmp(key, "webserver") == 0)
            setWebserverEnabled(VAL_BOOL(value));
        else if (strcmp(key, "webserverport") == 0)
            setWebserverPort(VAL_INT(value));
        else if (strcmp(key, "foreground") == 0)
            setForeground(VAL_BOOL(value));
        else if (strcmp(key, "resdir") == 0)
            setResourceDir(VAL_STR(value));
        else if (strcmp(key, "logfilename") == 0)
            Logger::SetOutputFile(VAL_STR(value));
        else if (strcmp(key, "authfile") == 0)
            setAuthFile(VAL_STR(value));
        else if (strcmp(key, "motdfile") == 0)
            setMOTDFile(VAL_STR(value));
        else if (strcmp(key, "rulesfile") == 0)
            setRulesFile(VAL_STR(value));
        else if (strcmp(key, "vehiclelimit") == 0)
            setMaxVehicles(VAL_INT(value));
        else if (strcmp(key, "owner") == 0)
            setOwner(VAL_STR(value));
        else if (strcmp(key, "website") == 0)
            setWebsite(VAL_STR(value));
        else if (strcmp(key, "irc") == 0)
            setIRC(VAL_STR(value));
        else if (strcmp(key, "voip") == 0)
            setVoIP(VAL_STR(value));
        else if (strcmp(key, "serverlist-host") == 0)
            s_serverlist_host = VAL_STR(value);
        else if (strcmp(key, "serverlist-path") == 0)
            s_serverlist_path = VAL_STR(value);
        else if (strcmp(key, "verbosity") == 0)
            Logger::SetLogLevel(LOGTYPE_DISPLAY, (LogLevel)VAL_INT(value));
        else if (strcmp(key, "logverbosity") == 0)
            Logger::SetLogLevel(LOGTYPE_FILE, (LogLevel)VAL_INT(value));
        else if (strcmp(key, "heartbeat-interval") == 0)
            setHeartbeatIntervalSec(VAL_INT(value));
        else

...
void Sequencer::queueMessage(int uid, int type, unsigned int streamid, char *data, unsigned int len)
{
    ...
    else if (type == RoRnet::MSG2_STREAM_REGISTER)
    {
        ...
        else
        {
            ...
            if (publishMode != BROADCAST_BLOCK)
            {
                ...
                {
                    if (reg->name[i] == ' ')
                        reg->name[i] = 0;                                                                                                                                                                                                                                                                                                                                          // convert spaces to zero's
                }
                reg->name[127]            = 0;
    ...
    else if (type == RoRnet::MSG2_UTF8_CHAT)
    {
        ...
        if (m_script_engine)
        {
            ...
            if (scriptpub != BROADCAST_AUTO)
                publishMode = scriptpub;
        }
#endif  //WITH_ANGELSCRIPT
        if (str == "!help")
        {
        ...
        else if (str == "!list")
        {
            ...
            for (unsigned int i = 0; i < m_clients.size(); i++)
            {
                if (i >= m_clients.size())
                    break;
                char authst[10] = "";
                if (m_clients[i]->user.authstatus & RoRnet::AUTH_ADMIN)
                    strcat(authst, "A");
                if (m_clients[i]->user.authstatus & RoRnet::AUTH_MOD)
                    strcat(authst, "M");
                if (m_clients[i]->user.authstatus & RoRnet::AUTH_RANKED)
                    strcat(authst, "R");
                if (m_clients[i]->user.authstatus & RoRnet::AUTH_BOT)
                    strcat(authst, "B");
                if (m_clients[i]->user.authstatus & RoRnet::AUTH_BANNED)
                    strcat(authst, "X");  \
...
#ifdef WITH_ANGELSCRIPT
        if (m_script_engine)
            m_script_engine->gameCmd(client->user.uniqueid, std::string(data));
#endif  //WITH_ANGELSCRIPT
        publishMode = BROADCAST_BLOCK;
    }
#if 0
